...
export function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#f5f3f0]/95 backdrop-blur-sm border-b border-[#e8e2d9]/30">
      <div className="container mx-auto px-4 sm:px-6 py-2 sm:py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-1 sm:space-x-3">
            ...
            {/* Language Selector - Tek sıra, net isimler */}
            <div className="relative">
              ...
              <AnimatePresence>
                {isLanguageOpen && (
                  ...
                  >
                    {/* Tek sıra layout - net isimler */}
                    <div className="flex flex-col p-2 min-w-[140px]">
                      {languages.map((lang) => (
                        <button
                          key={lang.code}
                          className="text-left px-3 py-2 text-sm text-shinest-blue hover:bg-[#f5f3f0] hover:text-shinest-blue/80 transition-colors duration-200 rounded-md whitespace-nowrap"
                          onClick={() => {
            ...
            <Button
              variant="ghost"
              className="text-shinest-blue hover:text-shinest-blue/80 w-12 h-12 sm:w-14 sm:h-14 rounded-full hover:bg-[#e8e2d9]/50 transition-all duration-300 ml-1 sm:ml-2 flex flex-col items-center justify-center gap-1 border border-[#e8e2d9]/30 z-[60]"
      ...
      {/* Tam Ekran Menü - Tamamen Beyaz Arka Plan - Yüksek Z-Index */}
      <AnimatePresence>
        {isMenuOpen && (
          <>
            {/* Backdrop */}
            ...
            >
              <div className="container mx-auto px-6 py-6 h-screen flex flex-col bg-white">
                ...
                <div className="flex justify-between items-center flex-shrink-0 bg-white">
                  ...
                  <Button
                    variant="ghost"
                    className="text-shinest-blue hover:text-shinest-blue/80 w-12 h-12 sm:w-14 sm:h-14 rounded-full hover:bg-[#e8e2d9]/50 transition-all duration-300 border border-[#e8e2d9]/30 z-[60]"
                  ...
                  <nav className="text-center">
                    {menuItems.map((item, index) => (
                      ...
                      >
                        <Link
                          href={item.href}
                          className="font-display text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-shinest-blue hover:text-shinest-blue/80 transition-colors duration-300 block py-2"
                          onClick={() => setIsMenuOpen(false)}
                        >
                          {item.name}
                        </Link>
                ...
                >
                  <Link
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-shinest-blue hover:text-shinest-blue/80 w-12 h-12 sm:w-14 sm:h-14 rounded-full hover:bg-[#e8e2d9]/50 transition-all duration-300 border border-[#e8e2d9]/30 flex items-center justify-center"
                  >
